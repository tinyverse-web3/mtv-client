...
console.log(ethers);
export enum KYEMANAGER_STATUS_CODE {
  EMPTY_PASSWORD,
  INVALID_PASSWORD,
  EMPTY_KEYSTORE,
  SUCCESS,
...
export class KeyManager {
  ...
  async restoreFromEntropy(entropy: string, password: string) {
    ...
    await this.saveKeyStore(password);
    await this.exposed();
    return KYEMANAGER_STATUS_CODE.SUCCESS;
  }
  ...
  async changePwd(oldPwd: string, newPwd: string) {
    ...
    if (status === KYEMANAGER_STATUS_CODE.SUCCESS && this.wallet) {
      // await this.password.removeSalt();
      await this.password.set(newPwd);
      // console.log('新密码hash', encryptPwd);
      const { entropy } = this.wallet.mnemonic || {};
      if (entropy) {
        await this.keystore.create(entropy, newPwd);
      }
    }
    return status;
  }
  ...
  private async _verify(encryptPwd: string) {
    try {
      ...
      if (keystore) {
        ...
        return KYEMANAGER_STATUS_CODE.INVALID_PASSWORD;
      }
    } catch (error) {
      console.log(error);
      return KYEMANAGER_STATUS_CODE.INVALID_PASSWORD;
  ...
  async verify(password: string) {
    ...
    return await this._verify(password);
  }
  ...
  async check() {
    ...
    if (!keystore) {
      return KYEMANAGER_STATUS_CODE.EMPTY_KEYSTORE;
    }
    const password = await this.password.get();
    if (!password) {
      return KYEMANAGER_STATUS_CODE.EMPTY_PASSWORD;
    }
